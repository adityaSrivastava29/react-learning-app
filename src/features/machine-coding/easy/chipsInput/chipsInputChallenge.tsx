...
export const ChipsInputChallenge: React.FC = () => {
  const { theme } = useTheme();
  const [chipsData, setChipsData] = useState<ChipItem[]>([
  ...
  const removeChip = (idToRemove: number) => {
    setChipsData((prev) => prev.filter((item) => item.id !== idToRemove));
  };

  const toggleActive = (idToToggle: number) => {
    setChipsData((prev) =>
      prev.map((chip) =>
